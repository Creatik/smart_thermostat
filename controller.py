...
class SmartOffsetController:
    ...
    async def async_start(self):
        """Запуск контроллера."""
        # Загружаем историю из storage (БЕЗ await, так как get_history синхронный)
    ...
    async def async_stop(self):
        ...
        await self.storage.set_heating_rate(self.entry.entry_id, self._heating_rate, reason="shutdown")
    ...
    async def _handle_deadband(self, data: Dict[str, Any], deadband: float) -> bool:
        ...
        target_changed = (self._last_room_target is not None and 
                         abs(t_target - self._last_room_target) > 1e-9)
    ...
    async def _handle_stable_learning(self, data: Dict[str, Any], deadband: float):
        ...
        long_window_open = (self.window_is_open and self._window_open_since and 
                           (now_mono - self._window_open_since >= window_no_learn_min))
    ...
    async def _handle_active_control(self, data: Dict[str, Any]):
        ...
        if self._heating_rate > 0.001:
            ...
            if predicted_time < predict_minutes:
                ...
                correction *= factor
                LOGGER.debug("Overshoot prevention: predicted_time=%.1f min, correction factor=%.2f", 
                            predicted_time, factor)
        ...
        if stuck_enable and not self.window_is_open and not self.boost_active:
            await self._handle_stuck_detection(
                t_room, e, deadband, stuck_seconds, stuck_min_drop, stuck_step, 
                now_mono, t_trv, step_min, trv_min, trv_max
            )
        ...
        if success:
            ...
            LOGGER.debug(
                ...
                data['climate_entity'], t_room, t_target, e, offset, correction, t_trv, self._heating_rate
            )
    ...
    async def _handle_stuck_detection(self, t_room: float, e: float, deadband: float,
                                     stuck_seconds: int, stuck_min_drop: float, stuck_step: float,
                                     now_mono: float, t_trv: float, step_min: float,
                                     trv_min: float, trv_max: float):
        ...
        if self._prev_room_temp is not None and self._prev_time is not None:
            ...
            if dt > 0:
                ...
                if dT > 0:  # только подъём температуры
                    ...
                    # Периодически сохраняем heating rate
                    if now_mono % 300 < 5:  # каждые ~5 минут
