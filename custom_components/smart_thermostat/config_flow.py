    ...
    async def async_step_user(self, user_input=None):
        ...
        schema = vol.Schema({
            ...
            vol.Required(CONF_ROOM_SENSORS): EntitySelector(
                EntitySelectorConfig(domain="sensor", multiple=True, device_class="temperature")
            ),
...
class SmartThermostatOptionsFlow(config_entries.OptionsFlow):
    ...
    async def async_step_init(self, user_input=None):
        ...
        schema = vol.Schema({
            ...
            vol.Optional(
                CONF_WEATHER_FORECAST_TYPE,
                default=get_option(CONF_WEATHER_FORECAST_TYPE, "hourly")
            ): SelectSelector(
                SelectSelectorConfig(
            ...
            vol.Optional(
                CONF_ECO_OFFSET,
                default=get_option(CONF_ECO_OFFSET, -3.0)
            ): NumberSelector(
                NumberSelectorConfig(
                    min=-10.0, max=0.0, step=0.5,
                    mode=NumberSelectorMode.BOX, unit_of_measurement="°C"
                )
            ),
            ...
            vol.Optional(
                CONF_PREDICTIVE_FACTOR,
                default=get_option(CONF_PREDICTIVE_FACTOR, 0.3)
            ): NumberSelector(
                NumberSelectorConfig(
            ...
            vol.Optional(
                CONF_PID_KP,
                default=get_option(CONF_PID_KP, 0.6)
            ): NumberSelector(
                NumberSelectorConfig(min=0.1, max=2.0, step=0.1, mode=NumberSelectorMode.BOX)
            ),

            vol.Optional(
                CONF_PID_KI,
                default=get_option(CONF_PID_KI, 0.05)
            ): NumberSelector(
                NumberSelectorConfig(min=0.0, max=0.5, step=0.01, mode=NumberSelectorMode.BOX)
            ),

            vol.Optional(
                CONF_PID_KD,
                default=get_option(CONF_PID_KD, 0.1)
            ): NumberSelector(
                NumberSelectorConfig(min=0.0, max=1.0, step=0.05, mode=NumberSelectorMode.BOX)
            ),
            vol.Optional(
                CONF_COOLDOWN_REDUCTION_FACTOR,
                default=get_option(CONF_COOLDOWN_REDUCTION_FACTOR, 0.3)
            ): NumberSelector(
                NumberSelectorConfig(
                    min=0.0, max=0.8, step=0.05,
                    mode=NumberSelectorMode.BOX,
                    unit_of_measurement="коэфф."
                )
            ),
            # Влажность
            vol.Optional(
