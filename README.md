...
---
### Состояния сенсора "Последнее действие"

Сенсор показывает, что именно сделал контроллер в последнем цикле. Это помогает понять, почему батарея не остывает или почему ничего не происходит.

| Состояние                    | Когда происходит                                      | Что значит                                                                 |
|------------------------------|-------------------------------------------------------|----------------------------------------------------------------------------|
| Инициализация                | При запуске контроллера                               | Контроллер только стартовал, подождите 1–2 минуты                          |
| В мёртвой зоне — удержание   | Температура в пределах deadband                       | Всё стабильно, ничего менять не нужно                                      |
| Уставка изменена             | Отправлена новая температура на радиатор              | Контроллер работает — батарея должна реагировать                           |
| Изменение не требуется       | Уставка почти не изменилась                           | Экономим батарею TRV и сеть                                                |
| Нет датчиков температуры     | Нет настроенных датчиков комнаты                      | **Проблема**: добавьте датчики в настройках интеграции                     |
| Нет валидных показаний       | Датчики недоступны или показывают не число            | **Проблема**: проверьте датчики в States (должны быть числами)             |
| Термостат недоступен         | Реальный радиатор unavailable                         | **Проблема**: проверьте связь с радиатором (Zigbee, батарея)               |
| Окно открыто                 | Открыто окно → уставка = trv_min                      | Проветривание — батарея остывает                                           |
| Детектор залипания: снижено  | Перегрев → принудительно снижена уставка              | Контроллер "вылечил" залипание клапана                                     |
| Стабильное обучение          | Температура стабильна → offset подкорректирован       | Медленное обучение смещения                                                |
| Смещение сброшено            | Нажата кнопка "Сбросить смещение"                     | Offset = 0, уставка ближе к цели                                           |
| Неизвестно                   | Ошибка или не удалось определить действие             | Редко. Обычно после рестарта. Подождите или перезагрузите интеграцию       |

**Совет**: если часто видите "Нет валидных показаний" или "Термостат недоступен" — проверьте датчики и радиатор в **Developer Tools → States**.
